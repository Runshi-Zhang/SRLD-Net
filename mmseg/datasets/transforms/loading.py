...
class LoadBiomedicalAnnotation(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        long = 2 * r + 1
        w = 3 * r + 1
        guss_size = np.zeros([long, long, w])
        for x in range(long):
            for y in range(long):
                for z in range(w):
                    guss_size[x, y, z] = np.exp(-((x - r) ** 2 + (y - r) ** 2 + (z - 36) ** 2) / (2 * sigma ** 2))
        '''

        pik = open(results['seg_map_path'], 'rb')
        import pickle
        inf = pickle.load(pik)
        name = results['seg_map_path'][-12:-4]
        spacing = inf['rawspacing' + name]
        #pointcenter = inf['rawlocation'+name]
        results['spacing'] = spacing
        results['location'] = inf['point'+name]
        results['oxy'] = inf['oxy' + name]
        results['name'] = inf['name']
        #results['point'] = inf['point'+name]
        filename = results['seg_map_path'][:-4]+'.npy'
