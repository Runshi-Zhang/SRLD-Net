...
class EightMetric(BaseMetric):
        ...
        for data_sample in data_samples:
            ...
            if not self.format_only:
                label = data_sample['gt_sem_seg']['data'].squeeze().to(
                    pred_label)
                self.results.append(
                    self.intersect_and_union(pred_label, label, data_sample['location'], data_sample['spacing'], data_sample['seg_map_path'][-12:-4], data_sample['oxy'], data_sample['name']))
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        result = np.zeros([2,6,re.shape[1]])
        ...
        metrics['SDR2'] = max(np.sum(result[0, :, :] < 2), 1) / (6 * re.shape[1])
        metrics['SDR3'] = max(np.sum(result[0, :, :] < 3), 1) / (6 * re.shape[1])
        metrics['SDR4'] = max(np.sum(result[0, :, :] < 4), 1) / (6 * re.shape[1])
        metrics['aftermean'] = np.mean(result[1, :, :])
        metrics['afterstd'] = np.std(result[1, :, :])
        metrics['afterSDR2'] = max(np.sum(result[1, :, :] < 2), 1) / (6 * re.shape[1])
        metrics['afterSDR3'] = max(np.sum(result[1, :, :] < 3), 1) / (6 * re.shape[1])
        metrics['afterSDR4'] = max(np.sum(result[1, :, :] < 4), 1) / (6 * re.shape[1])
        return metrics
    ...
    @staticmethod
    def savelocation(point, name, spacing, oxy):
        with open('/home/zrs/result/' + name + '.txt', 'w') as f:
            for i in range(6):
                #f.write(str(point[i,0] * spacing[0] + oxy[0]) + '\t' + str(point[i,1] * spacing[1] + oxy[1]) + '\t' + str(point[i,2] * spacing[2] + oxy[2]) + '\n')
                f.write(str(point[i, 0]) + '\n')
    def intersect_and_union(self, pred_label: torch.tensor, label: torch.tensor,
                            location,spacing, name, oxy, truename):
        ...
        for i in range(6):
            ...
            result[i,0] = np.sqrt(np.square(prepoint[0][0] * spacing[0] - location[i,0]) +np.square(prepoint[1][0] * spacing[1] - location[i,1]) + np.square(prepoint[2][0] * spacing[2] - location[i,2]))
            result[i, 1] = (np.abs(prepoint[0][0] * spacing[0] - location[i,0]) + np.abs(prepoint[1][0] * spacing[1] - location[i,1]) + np.abs(prepoint[2][0] * spacing[2] - location[i,2])) / 3.0
            result[i, 2] = (np.square(prepoint[0][0] * spacing[0] - location[i,0]) +np.square(prepoint[1][0] * spacing[1] - location[i,1])) + np.square(prepoint[2][0] * spacing[2] - location[i,2])/3.0
        ...
        self.savepoint(result, truename, spacing)
        #self.savelocation(blurkeypoint,truename,spacing,oxy)
