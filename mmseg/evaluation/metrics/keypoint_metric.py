...
class TenMetric(BaseMetric):
        ...
        for data_sample in data_samples:
            ...
            if not self.format_only:
                label = data_sample['gt_sem_seg']['data'].squeeze().to(
                    pred_label)
                self.results.append(
                    self.intersect_and_union(pred_label, label, data_sample['location'], data_sample['spacing'],
                                             data_sample['seg_map_path'][-12:-4], data_sample['oxy'],
                                             data_sample['name']))
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        result = np.zeros([2,10,re.shape[1]])
