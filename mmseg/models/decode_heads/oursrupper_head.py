...
class OurSrUperHead(BaseDecodeHead):
    ...
    def __init__(self, pool_scales=(1, 2, 3, 6),
                 ...
                 upsample_log=[0, 1, 2, 3],
                 per_emb_nums=[1, 4, 8, 16],
        ...
        self.fpn_bottleneck = ConvModule(
            (len(self.in_channels)-0) * self.channels,
            self.channels,
            3,
            padding=1,
            conv_cfg=self.conv_cfg,
        ...
        self.kp_encoder = nn.ModuleList([nn.Sequential(
            nn.Conv3d(self.channels, per_emb_num * self.num_classes, 1),
        ...
        self.converge = nn.ModuleList([ConvergeHead(self.per_emb_nums[i], 2 ** self.upsample_log[i], 9,
                                                    4 , self.num_classes) if self.supervises[i] else nn.Identity() for i
                                       in range(num)])

        self.kp_encoderfinal = nn.ModuleList([nn.Sequential(
            nn.Conv3d(self.channels, 4 * self.num_classes, 1),
            # nn.BatchNorm2d(per_emb_num * num_joints),
            nn.ReLU()
        )])
        num = len(self.out_channels_all)
        self.convergefinal = nn.ModuleList([ConvergeHead(4, 2 ** 2, 3,
                                                    1, self.num_classes)])
        self.fuse_two = nn.ModuleList(
            [nn.Sequential(*[ConvBlock(self.num_classes) for _ in range(2)])])
    ...
    def _forward_feature(self, inputs):
        ...
        laterals.append(self.nopsp(inputs[-1]))
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            fpn_outs[i] = resize(
                ...
                mode='trilinear',
                align_corners=self.align_corners)
        #fpn_outs = torch.cat(fpn_outs[:4], dim=1)
        #feats = self.fpn_bottleneck(fpn_outs)


        feats = self.kp_encoderfinal[-1](fpn_outs[0])
        feats = self.convergefinal[-1](feats)
