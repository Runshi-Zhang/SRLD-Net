...
class SRUNet(BaseModule):
    ...
    def __init__(self,
        ...
        self.psp_modules = PPM(
            (1, 2, 3, 6),
            self.base_channels * 2**(self.num_stages-1),
            512,
            conv_cfg=conv_cfg,
            norm_cfg=norm_cfg,
            act_cfg=act_cfg,
        ...
        self.bottleneck = ConvModule(
            self.base_channels * 2**(self.num_stages-1) + 4 * 512,
            self.base_channels * 2**(self.num_stages-1),
            3,
            padding=1,
            conv_cfg=conv_cfg,
        ...
        for i in range(num_stages):
            ...
            if i != 0:
                if strides[i] == 1 and downsamples[i - 1]:
                    enc_conv_block.append(nn.MaxPool3d(kernel_size=2))
    ...
    def forward(self, x):
        ...
        for enc in self.encoder:
            x = enc(x)
            enc_outs.append(x)
        #laterals.append(self.psp_forward(enc_outs))
        dec_outs = [self.psp_forward(enc_outs)]
    ...
    def _check_input_divisible(self, x):
        ...
        for i in range(1, self.num_stages):
            if self.strides[i] == 2 or self.downsamples[i - 1]:
                whole_downsample_rate *= 2
        assert (h % whole_downsample_rate == 0) \
            and (w % whole_downsample_rate == 0) and (z % whole_downsample_rate == 0),\
