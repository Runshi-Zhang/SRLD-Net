...
def channel_shuffle(x, groups):
    ...
    batch_size, num_channels, height, width, Long = x.size()
    assert (num_channels % groups == 0), ('num_channels should be '
                                          'divisible by groups')
    channels_per_group = num_channels // groups

    x = x.view(batch_size, groups, channels_per_group, height, width, Long)
...
class CrossResolutionWeighting(BaseModule):
    ...
    def forward(self, x):
        ...
        out = [
            s * F.interpolate(a, size=s.size()[-3:], mode='trilinear')
